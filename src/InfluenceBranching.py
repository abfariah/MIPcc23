...
class InfluenceBranching(scip.Branchrule):
    def __init__(
        ...
        self.obj_1 = False
        self.normalize = True  # To be adjusted
    ...
    def get_true_A(self) -> np.array:
        ...
        for cons in self.constraints:
            if cons.isLinear():  ## check constraint is linear
    ...
    def get_matrices(self) -> Tuple[np.array, np.array, np.array]:
        ...
        self.idx_binary = np.where(binary_condition)[0]
        ...
        b = np.array(
            [
                [
                    self.model.getRhs(self.constraints[i])
                    for i in range(self.m)
                    if self.constraints[i].isLinear()
                ]
            ]
        )
        b = b.reshape(-1, 1)
    ...
    def std_model(self) -> Tuple[np.array, np.array, np.array]:
        ...
        """
        A, b, c = self.get_matrices()
        if self.normalize:
            A, b, c = self.std_matrices(A, b, c)
    ...
    def get_best_candidate(self, action_set: np.array) -> np.array:
        ...
        if self.obj_1:
            variables_influence *= np.log(1 + self.c[idx_actions])
        idx_best_candidate = action_set[np.argsort(-variables_influence)[0]]
        del W_n
